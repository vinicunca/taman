...
export function useTiptapToolbar(options: UseTiptapToolbarOptions) {
  ...
  function getToolbarButtonClass(action: ToolbarAction) {
    return cn(
      ...
      'enabled:hover:bg-background-accented enabled:hover:color-text',
      isActionActive(action) &&
        'bg-background-accented border-primary/30 shadow-primary text-primary',
    );
  }
  ...
  function getMenuItemClass(item: ToolbarMenuItem) {
    return cn(
      ...
      isMenuItemActive(item)
        ? 'bg-background-accented color-text'
        : 'hover:bg-background-accented hover:color-text text-muted-foreground',
    );
  }
