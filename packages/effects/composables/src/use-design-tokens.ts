...
import { convertToRgb, updateCSSVariables } from '@taman/utils';

/**
 * Adapts design tokens for each UI framework.
 */
...
export function useAntdDesignTokens() {
  ...
  const tokens = reactive({
    ...
    colorTextBase: '',
    colorWarning: '',
    zIndexPopupBase: 2000, // Raise base popup z-index so dropdowns are not covered by modals or maximized tables
  });
  ...
  watch(
    ...
    () => {
      ...
      const variables: Record<string, string> = {
        ...
        // Fix ElLoading overlay background color
