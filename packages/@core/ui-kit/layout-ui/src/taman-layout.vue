...
// Whether the side is in the hover state of the expanded menu
...
/**
 * The height of the sidebar area from the top
 */
...
/**
 * Whether the sidebar mode, including mixed sidebar
 */
...
const tabbarStyle = computed<CSSProperties>(() => {
  ...
  // If it is not a mixed navigation, the width of the tabbar is 100%
  if (!isMixedNav.value || props.sidebarHidden) {
    width = '100%';
  } else if (sidebarEnable.value) {
    // When the mouse is on the sidebar, and the sidebar is expanded, the width
    ...
    // Set the width of the tabbar, the calculation method is 100% minus the width of the sidebar
...
<template>
  <div class="flex min-h-full w-full relative">
    ...
    <div
      v-if="maskVisible"
      :style="maskStyle"
      class="bg-overlay size-full transition-[background-color] duration-200 left-0 top-0 fixed"
