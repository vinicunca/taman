...
import type { ClassType } from '@taman-core/typings';
...
function onAnimationEnd(event: AnimationEvent) {
  // Only emit opened/closed when contentRef's animation finishes
  ...
  <Teleport defer :to="appendTo">
    ...
    >
      ...
      <DialogClose
        ...
        :class="
          cn(
            'flex-center color-text/80 hover:bg-background-accented hover:text-accent-foreground data-[state=open]:bg-background-accented data-[state=open]:text-muted-foreground absolute top-3 right-3 h-6 w-6 rounded-full px-1 text-lg opacity-70 transition-opacity hover:opacity-100 focus:outline-hidden disabled:pointer-events-none',
            props.closeClass,
          )
        "
        @click="() => emits('close')"
