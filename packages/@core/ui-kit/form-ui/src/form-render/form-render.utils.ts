...
import type { FormLabelWidthContext } from '../form.types';
...
export function useFormLabelWidth() {
  const potentialLabelWidthArr = ref<Array<number>>([]);

  const autoLabelWidth = computed(() => {
    if (potentialLabelWidthArr.value.length === 0) {
      return '0';
    }
...
export interface FormResolveLabelStyleInput {
...
export function formResolveLabelStyle(
  input: FormResolveLabelStyleInput,
...
export function useFieldLabelWidth(
  options: {
  ...
  function updateLabelWidth(action: 'remove' | 'update' = 'update') {
    nextTick(() => {
