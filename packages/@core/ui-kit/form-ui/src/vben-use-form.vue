...
import { clone, get, isEqual, set } from '@taman-core/shared/utils';
...
// Duplicated props instead of extends to avoid HMR freeze
...
function handleKeyDownEnter(event: KeyboardEvent) {
  ...
  // Do not prevent default for textarea so line breaks work.
  // Skip Enter-to-submit for textarea
...
onMounted(async () => {
  // Watch only after mount; form.values initializes first
  await nextTick();
  watch(
    () => form.values,
    async (newVal) => {
      if (forward.value.handleValuesChange) {
        ...
        if (fields && fields.length > 0) {
          ...
          if (changedFields.length > 0) {
            // Call handleValuesChange with a deep copy of values and changed fields
            const values = await forward.value.formApi?.getValues();
            forward.value.handleValuesChange(
              clone(values ?? {}) as Record<string, any>,
              changedFields,
            );
          }
        }
