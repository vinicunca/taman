...
export class FormApi {
  ...
  /**
   * Component instance map by field name
   */
  private componentRefMap: Map<string, unknown> = new Map();

  // Form values from the last submit click
  ...
  /**
   * Get the component instance for a field
  ...
  /**
   * Get the currently focused field, or undefined if none is focused
   */
  getFocusedField() {
  ...
  async getValues<T = Recordable<any>>() {
    ...
    const values = form.values
      ? this.handleRangeTimeValue(clone(toRaw(form.values)))
      : {};
  ...
  mount(formActions: FormActions, componentRefMap?: Map<string, unknown>) {
    if (!this.isMounted) {
      ...
      const initialValues = this.form.values
        ? this.handleRangeTimeValue(clone(toRaw(this.form.values)))
        : {};
  ...
  scrollToFirstError(errors: Record<string, any> | string) {
    ...
    // Fallback: find via component ref if name attribute lookup fails (vee-validate may change name)
    if (!el) {
    ...
    if (el) {
      // Scroll to the error field with center alignment so it is fully visible
      el.scrollIntoView({
        behavior: 'smooth',
        block: 'center',
        inline: 'nearest',
   ...
   * Set form values
   * @param fields record
   * @param filterFields Omit fields not defined in schema; default true
  ...
  async setValues(
    ...
    /**
     * Merge logic may need improvement; object values are not fully supported.
     * Ant Design date/time components use dayjs values.
     * Element Plus date/time components may use Date values.
     * Both types are excluded from deep merge.
     */
  ...
  updateSchema(schema: Array<Partial<FormSchema>>) {
    const updated: Array<Partial<FormSchema>> = [...schema];
  ...
  private handleMultiFields = (originValues: Record<string, any>) => {
    ...
    const processFields = (fields: Array<string>, separator: string = ',') => {
    ...
    // Simple array format: ['field1', 'field2', ';'] or ['field1', 'field2']
    ...
    // Nested array format: [['field1'], ';']
    arrayToStringFields.forEach((fieldConfig) => {
      if (Array.isArray(fieldConfig)) {
        const [fields, separator = ','] = fieldConfig;
        // Per type definition, fields must always be a string array
