...
import type { FormSchemaRuleType, TamanFormFieldSlotProps } from '../src/form.types';
...
describe('useTamanForm integration', () => {
  ...
  it('keeps only the active model protocol in field slot componentProps', async () => {
    ...
    const [Form, formApi] = useTamanForm<ModelProtocolValues>({
      schema: [
        {
          component: TestInput,
          componentProps: {
    ...
    if (!defaultSlotProps || !valueSlotProps) {
      return;
    }
  ...
  it('groups control bindings in field slot componentProps', async () => {
    ...
    let latestSlotProps:
      | undefined
      | TamanFormFieldSlotProps<SlotFormValues, 'name'>;
    const [Form, formApi] = useTamanForm<SlotFormValues>({
      schema: [
        {
          component: TestInput,
          defaultValue: 'Ada',
    ...
    const wrapper = mount(Form, {
      slots: {
        name(slotProps: TamanFormFieldSlotProps<SlotFormValues, 'name'>) {
  ...
  it('normalizes disabled state in field slot componentProps', async () => {
    ...
    const [Form] = useTamanForm<DisabledFormValues>({
