...
export function useTabsViewScroll(props: TabsProps) {
  ...
  async function initScrollbar() {
    ...
    const viewportEl = scrollbarEl?.querySelector(
      'div[data-akar-scroll-area-viewport]',
    );
    ...
    resizeObserver = new ResizeObserver(
      useDebounceFn((_entries: Array<ResizeObserverEntry>) => {
        calcShowScrollbarButton();
        scrollToActiveIntoView();
      }, 100),
    );
    ...
    // Use MutationObserver to watch child count only
    ...
    // Observe only child node additions and removals
  ...
  /**
   * Compute tab widths to decide whether scroll buttons are shown
   */
  ...
  watch(
    () => props.active,
    async () => {
      // 200ms delay to wait for tab switch animation
      // setTimeout(() => {
      scrollToActiveIntoView();
      // }, 300);
    },
