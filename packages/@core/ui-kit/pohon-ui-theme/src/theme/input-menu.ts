  ...
  slots: defu(
    {
      ...
      trailing: 'group absolute inset-y-0 end-0 flex items-center disabled:(cursor-not-allowed opacity-75) focus:outline-none',
      trailingClear: 'p-0',
      arrow: 'fill-bg stroke-default',
      content: 'max-h-[min(15rem,var(--akar-combobox-content-available-height))] w-$akar-combobox-trigger-width bg-background shadow-lg rounded-md ring ring-ring overflow-hidden data-[state=closed]:(animate-out fade-out-0 zoom-out-95) data-[state=open]:(animate-in fade-in-0 zoom-in-95) origin-$akar-combobox-content-transform-origin pointer-events-auto flex flex-col',
      viewport: 'relative scroll-py-1 overflow-y-auto flex-1',
      group: 'p-1 isolate',
      empty: 'text-center color-text-muted',
      label: 'font-600 color-text-highlighted',
      separator: '-mx-1 my-1 h-px bg-border',
      item: 'group relative w-full flex items-start gap-1.5 p-1.5 text-sm select-none outline-none before:(absolute -z-1 inset-px rounded-md) data-[disabled]:cursor-not-allowed data-[disabled]:opacity-75 color-text data-[highlighted]:not-[[data-disabled]]:color-text-highlighted data-[highlighted]:not-[[data-disabled]]:before:bg-background-elevated/50 transition-colors before:transition-colors',
