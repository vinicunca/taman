...
describe('storageManager', () => {
  ...
  it('should clear expired items', async () => {
    await storageManager.setItem('user', { age: 30, name: 'John Doe' }, 1000); // expires in 1 second
  ...
  it('should not clear non-expired items', async () => {
    await storageManager.setItem('user', { age: 30, name: 'John Doe' }, 10_000); // expires in 10 seconds
    vi.advanceTimersByTime(5000); // advance time
    await storageManager.clearExpiredItems();
    const user = await storageManager.getItem('user');
    expect(user).toEqual({ age: 30, name: 'John Doe' });
  });
  ...
  it('should handle multiple items with different expiry times', async () => {
    await storageManager.setItem('user1', { age: 30, name: 'John Doe' }, 1000); // expires in 1 second
    await storageManager.setItem('user2', { age: 25, name: 'Jane Doe' }, 2000); // expires in 2 seconds
    vi.advanceTimersByTime(1500); // advance time
    await storageManager.clearExpiredItems();
  ...
  it('should handle items with no expiry', async () => {
    await storageManager.setItem('user', { age: 30, name: 'John Doe' });
    vi.advanceTimersByTime(10_000); // advance time
    await storageManager.clearExpiredItems();
    const user = await storageManager.getItem('user');
    expect(user).toEqual({ age: 30, name: 'John Doe' });
  });
