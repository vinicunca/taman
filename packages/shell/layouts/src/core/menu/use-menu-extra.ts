...
export function useMenuExtra(useRootMenus?: ComputedRef<Array<TamanMenuRecordRaw>>) {
  ...
  const extraRootMenus = ref<Array<TamanMenuRecordRaw>>([]);
  const route = useRoute();
  const extraMenus = ref<Array<TamanMenuRecordRaw>>([]);
