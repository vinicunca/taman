...
export function useMenuMixed() {
  const { navigation, willOpenedByWindow } = useNavigation();
  const accessStore = useAccessStore();
  const route = useRoute();
  const splitSideMenus = ref<Array<TamanMenuRecordRaw>>([]);
  const rootMenuPath = ref<string>('');
  const mixedRootMenuPath = ref<string>('');
  const mixExtraMenus = ref<Array<TamanMenuRecordRaw>>([]);
   ...
   */
  const handleMenuOpen = (key: string, parentsPath: Array<string>) => {
