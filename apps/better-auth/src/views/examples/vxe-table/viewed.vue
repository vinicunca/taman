...
const [Grid, gridApi] = useVbenVxeGrid({
  gridOptions,
  viewedRowOptions: {
    // Opcode that triggers the viewed/read action
    actionCodes: ['view'],
    // Unique identifier field in row data
    keyField: 'id',
    // Persistence config (shorthand mode using built-in localStorage)
    // persist: 'viewed_rows',
    persist: {
      key: 'viewed-rows',
      type: 'indexedDB',
      ttl: 7 * 24 * 60 * 60 * 1000, // Expires after 7 days
      maxSize: 200,
    },
  },
});
