...
const client = createFetchClient({
  baseURL: apiUrl,
  // Backend authenticates off the Better Auth session cookie and runs on a
  // different origin, so cookies have to ride along — ofetch defaults to
  // `same-origin` and would send none. Backend's CORS layer already replies
  // with `access-control-allow-credentials: true` for trusted origins.
  credentials: 'include',
});

/**
 * Envelope-aware: backend's `/api/*` routes reply `{ code: 0, data, message }`
 * (see its `defineApiHandler`), and this unwraps to `data` or throws
 * `ApiError`.
 *
 * Failures are NOT enveloped — backend returns `{ error, status, message }`
