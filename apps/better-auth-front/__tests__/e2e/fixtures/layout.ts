  ...
  layoutPage: [
    async ({ browser }, use) => {
      ...
      await context.addInitScript(() => {
        Object.defineProperty(window, '__TAMAN_LAYOUT_E2E__', {
          configurable: true,
          value: true,
        });
      });
      ...
      try {
        ...
        await page.waitForFunction(() => Boolean(window.__TAMAN_LAYOUT_TEST__));
        await use(page);
      } finally {
        await page
          .evaluate(async () => {
            await window.__TAMAN_LAYOUT_TEST__?.resetPreferences();
          })
          .catch(() => {});
        await context.close();
      }
