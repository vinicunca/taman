...
interface KnipFileIssue {
  dependencies: Array<KnipDependency>;
  devDependencies: Array<KnipDependency>;
  file: string;
  optionalPeerDependencies: Array<KnipDependency>;
}

interface KnipResult {
  issues: Array<KnipFileIssue>;
}
...
async function runKnipCheck(): Promise<void> {
  ...
  const tempDir = await mkdtemp(join(tmpdir(), 'tooling-check-dep-'));
