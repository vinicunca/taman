...
**Tech Stack:** oRPC 1.14.11 (`@orpc/contract|server|client|tanstack-query|publisher|cloudflare`), zod 4.6.5, Nitro 3 beta (h3 v2), Drizzle + Postgres, better-auth, Vue 3 + `@tanstack/vue-query` 5, pohon-ui, vitest 5, tsdown, pnpm catalogs.

**Spec:** `docs/superpowers/specs/2026-09-24-orpc-design.md`

## Global Constraints

- Every `@orpc/*` package pinned to **exactly `1.14.11`** in the pnpm catalog (`@orpc/cloudflare`/`@orpc/publisher` have no 1.15; mixing duplicates `@orpc/client`). Do not use the 2.0 beta.
...
- In 1.14.11 `ORPCError` has `code`, `message`, `data`, `defined` — **no `status`**. Use `safe()` + `isDefinedError()` for typed errors (a bare `catch (e)` narrows to `never`).
- Retry plugin is `RetryLinkPlugin` from `@orpc/client/plugins` (not `ClientRetryPlugin`). Query-key prefix option is `prefix` (not the internal `path`).
...
- Produces (all exported from `@vinicunca/taman-api-contract`):
  ...
  - `type TamanInputs`, `type TamanOutputs` (`InferRouterContractInputs/Outputs<TamanContract>`)
  - `type TamanClient<TClientContext extends ClientContext = Record<never, never>>`
...
```yaml
  ...
  '@orpc/client': 1.14.11
  '@orpc/cloudflare': 1.14.11
  '@orpc/contract': 1.14.11
  '@orpc/publisher': 1.14.11
  '@orpc/server': 1.14.11
  '@orpc/tanstack-query': 1.14.11
...
Run: `pnpm install`
Expected: installs `@orpc/contract@1.14.11`; no peer warnings about `@orpc/*` version mismatch.
...
In `apps/better-auth-back/package.json` `dependencies` add `"@orpc/publisher": "catalog:"`, `"@orpc/server": "catalog:"`, `"@vinicunca/taman-api-contract": "workspace:*"`. Run: `pnpm install`.
...
If `os.todo.use(authed)` does not typecheck in 1.14.11, replace `const todo = os.todo.use(authed)` with per-procedure `os.todo.list.use(authed).handler(...)` (the prototype verified that form).
...
- Produces (`@vinicunca/taman-request/orpc`): `type TamanFetch = (url: string, init: RequestInit) => Promise<Response>`; `interface TamanClientOptions { baseUrl: string; fetch?: TamanFetch; headers?: Record<string, string> | (() => Record<string, string> | Promise<Record<string, string>>) }`; `type TamanClientContext = RetryLinkPluginContext`; `type TamanRpcClient = TamanClient<TamanClientContext>`; `createTamanClient(options): TamanRpcClient`; `LIVE_RETRY: TamanClientContext`; re-exports `ORPCError`, `isDefinedError`, `safe`, and types `TamanInputs`, `TamanOutputs`, `TodoEvent`, `Todo`, `TodoPage`.
- Produces (`@vinicunca/taman-request/orpc-query`): `createTamanQueryUtils(client: TamanRpcClient)` returning `RouterUtils<TamanRpcClient>` with key prefix `'taman'`; `type TamanQueryUtils = ReturnType<typeof createTamanQueryUtils>`.
...
function inProcessFetch() {
  ...
  const fetch: TamanFetch = async (url, init) => {
    calls.push({ url, init });
    const { response } = await handler.handle(new Request(url, init), { prefix: '/api/rpc', context: {} });
...
describe('createTamanClient', () => {
  it('calls /api/rpc on the base url with credentials and custom headers', async () => {
    ...
    expect(new Headers(calls[0]?.init.headers).get('x-app')).toBe('demo');
    expect(todo.createdAt).toBeInstanceOf(Date);
...
describe('createTamanQueryUtils', () => {
  it('prefixes every key with "taman"', () => {
    const utils = createTamanQueryUtils(createTamanClient({ baseUrl: 'http://api.test', fetch: vi.fn() }));
    expect(utils.todo.key()[0]).toBe('taman');
    expect(utils.todo.get.queryKey({ input: { id: ID } })[0]).toBe('taman');
  });
});
```
...
const client = createTamanClient({
  ...
  fetch: async (url, init) => {
    const { response } = await rpcHandler.handle(new Request(url, init), {
      prefix: RPC_PREFIX,
...
export type TamanFetch = (url: string, init: RequestInit) => Promise<Response>;
...
export type TamanClientContext = RetryLinkPluginContext;
...
export function createTamanClient(options: TamanClientOptions): TamanRpcClient {
  const baseFetch: TamanFetch = options.fetch ?? ((url, init) => globalThis.fetch(url, init));
  ...
  const link = new RPCLink<TamanClientContext>({
    ...
    fetch: (url, init) => baseFetch(url, { ...init, credentials: 'include' }),
...
/**
 * TanStack Query helpers (`queryOptions`, `mutationOptions`, `liveOptions`,
 * `key()` …) for any TanStack framework adapter. Keys start with `'taman'`
 * so they never collide with the host app's own queries.
 */
export function createTamanQueryUtils(client: TamanRpcClient) {
  return createTanstackQueryUtils(client, { prefix: 'taman' });
}
...
Expected: PASS; tsc exits 0. If the `origin` option rejects a plain `string` type, cast: `origin: options.baseUrl.replace(/\/+$/, '') as \`http://${string}\``. If `utils.todo.key()[0]` is not `'taman'`, print the key and adjust the assertion to the actual prefixed position — the prefix must be present.
...
In `apps/better-auth-back/package.json` add dependency `"@orpc/cloudflare": "catalog:"` and devDependencies `"@cloudflare/workers-types": "catalog:"`, `"wrangler": "catalog:"`. Run `pnpm install`.
...
```ts
import type { DurablePublisher } from '@orpc/cloudflare';
import type { Publisher } from '@orpc/publisher';
import type { TodoEvent } from '@vinicunca/taman-api-contract';
import type { H3Event } from 'nitro';
import { MemoryPublisher } from '@orpc/publisher/memory';
 ...
 *
 * `@orpc/cloudflare` imports `cloudflare:workers`, so it is loaded lazily and
...
async function createTodoPublisher(env?: TodoPublisherEnv): Promise<TodoPublisher> {
  if (env?.TODO_PUBLISHER) {
    const { DurablePublisher } = await import('@orpc/cloudflare');
...
type PublisherObjectArgs = ConstructorParameters<typeof DurablePublisherObject>;
 ...
 */
export class TodoPublisherObject extends DurablePublisherObject {
  constructor(ctx: PublisherObjectArgs[0], env: PublisherObjectArgs[1]) {
    super(ctx, env, { resume: { enabled: true, seconds: 300 } });
  }
}
```
